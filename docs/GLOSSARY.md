...
| **CadMesh**           | DXF 엔티티 타입별 전문화된 렌더링 컴포넌트 집합        |
...
| **ExtrudeGeometry**   | Three.js 2D 형상을 3D로 돌출시키는 지오메트리 클래스   |
| **Extrusion**         | 2D 형상을 3D로 압출하는 변환 기법                      |
| **ezdxf**             | Python DXF 파일 파싱 라이브러리                        |
| **Hatch3DMesh**       | HATCH 엔티티를 3D 돌출로 렌더링하는 컴포넌트           |
...
| **ShadingMode**       | 3D 모델 음영 처리 방식 (flat/smooth/phong)             |
...
| **Worker Pool**    | WebWorker 인스턴스 풀로 병렬 작업 처리 최적화        |
...
| 버전  | 날짜       | 변경 내용                                                                                              |
| ----- | ---------- | ------------------------------------------------------------------------------------------------------ |
| 1.0.1 | 2026-01-26 | 모든 카테고리 용어 알파벳순 정렬 적용 (검색성 개선)                                                    |
| 1.0.0 | 2026-01-15 | Phase 2.1.4 신규 용어 추가 (CadMesh, ExtrudeGeometry, Hatch3DMesh, ShadingMode, Worker Pool)           |
