...
> **Status**: ✅ 완료 (100%)
> **Dependencies**: Phase 1 Foundation 완료 ✅
>
...
| Sub-Phase | 상태 | 설명                       |
| --------- | ---- | -------------------------- |
| 1.5.1     | ✅   | TeapotGeometry 기본 렌더링 |
| 1.5.2     | ✅   | 6가지 쉐이딩 모드 구현     |
| 1.5.3     | ✅   | Interactive Controls UI    |
| 1.5.4     | ✅   | OrbitControls 카메라 조작  |
...
Three.js 공식 예제인 [webgl_geometry_teapot](https://threejs.org/examples/#webgl_geometry_teapot)을 React Three Fiber 기반으로 구현하여 3D 렌더링 기초를 학습
...
| **GUI 방식**    | HTML Overlay (Tailwind CSS) | lil-gui 통합      |
| **상태 관리**   | useState (로컬 상태)        | Zustand 전역 상태 |
| **쉐이딩 모드** | 6가지 모드                  | 커스텀 쉐이더     |

### 1.3 기술적 제약

- TeapotGeometry는 `three/addons`에서 import 필요
- R3F 환경에서 Three.js 객체를 JSX로 래핑해야 함
...
├── types.ts                    # 타입 정의 (ShadingMode, TeapotConfig)
├── constants.ts                # 상수 정의 (DEFAULT_TEAPOT_CONFIG)
├── components/
│   ├── index.ts
...
```
User Input → TeapotControls → useState(config) → TeapotMesh → Three.js Render

상태 변경 예시:
Shading Mode Select → setConfig({ shadingMode }) → useMemo(material) → mesh.material
```
...
### 3.1 Phase 1.5.1: 기본 렌더링

- [✅] TeapotGeometry import 설정
- [✅] TeapotMesh 컴포넌트 생성
- [✅] Canvas 및 기본 조명 설정
- [✅] OrbitControls 통합

### 3.2 Phase 1.5.2: 쉐이딩 모드

- [✅] wireframe 모드 (MeshBasicMaterial)
- [✅] flat 모드 (MeshPhongMaterial, flatShading)
- [✅] smooth 모드 (MeshStandardMaterial)
- [✅] glossy 모드 (MeshPhysicalMaterial)
- [✅] textured 모드 (체커보드 텍스처)
- [✅] reflective 모드 (환경 반사)

### 3.3 Phase 1.5.3: Interactive Controls

- [✅] Tessellation 슬라이더 (1-50)
- [✅] Shading Mode 선택 드롭다운
- [✅] Lid/Body/Bottom 가시성 토글
- [✅] Auto Rotate 체크박스

### 3.4 Phase 1.5.4: 최적화

- [✅] React.memo로 리렌더링 방지
- [✅] useMemo로 Material/Geometry 캐싱
- [✅] HTML Overlay로 3D/UI 분리
...
| 상수명         | 값        | 설명                   |
| -------------- | --------- | ---------------------- |
| `tessellation` | 15 (기본) | 테셀레이션 레벨 (1-50) |
| `autoRotate`   | true      | Y축 자동 회전          |
| `size`         | 3         | TeapotGeometry 크기    |
...
| ---------------------- | -------------------- | ---- |
| 6가지 쉐이딩 모드 전환 | 수동 확인            | ✅   |
| Tessellation 슬라이더  | 수동 확인            | ✅   |
| Lid/Body/Bottom 토글   | 수동 확인            | ✅   |
| Auto Rotate 기능       | 수동 확인            | ✅   |
...
| 패키지               | 버전     | 용도                             |
| -------------------- | -------- | -------------------------------- |
| `three`              | ^0.181.0 | TeapotGeometry 포함              |
...
| 문서                                                                           | 역할           |
| ------------------------------------------------------------------------------ | -------------- |
| [Three.js Teapot Example](https://threejs.org/examples/#webgl_geometry_teapot) | 원본 예제      |
| [React Three Fiber Docs](https://docs.pmnd.rs/react-three-fiber)               | R3F 공식 문서  |
...
| Phase    | 관계 | 설명                      |
| -------- | ---- | ------------------------- |
| Phase 1  | 의존 | Foundation 완료 후 시작   |
| Phase 2A | 연관 | CADViewer에서 패턴 재사용 |
...
| 버전  | 날짜       | 변경 내용                                                                          |
| ----- | ---------- | ---------------------------------------------------------------------------------- |
| 0.0.0 | 2025-12-01 | 초기 구현 완료                                                                     |
| 0.0.1 | 2025-12-02 | 템플릿 적용 (Language Guidelines, TOC, 섹션 제목 영어(한국어) 형식, 체크리스트 ✅) |
| 0.0.2 | 2025-12-02 | 버전 가이드라인 적용 (0.0.x), Section 9 추가                                       |
...
_Phase 1.5 Implementation - Created: 2025-12-01_
